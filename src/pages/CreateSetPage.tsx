...
const PICKABLE: Phase[] = [
  ...BUILTIN_PHASES,
  ...RANDOM_POOL_PHASES.filter(
    (p) => !BUILTIN_PHASES.some((b) => b.rule === p.rule && b.constraint === p.constraint),
  ),
];
          ...
          <div className="flex flex-col gap-1.5">
            {PICKABLE.map((phase) => {
              ...
              return (
                ...
                >
                  <span
                    className={`w-5 h-5 rounded flex items-center justify-center border-2 flex-shrink-0 transition-colors ${on ? "border-white bg-white text-bg-card" : "border-white/20"}`}
                  >
                  ...
                  {phase.difficulty && (
                    <span
                      className={`text-xs flex-shrink-0 ${DIFF_COLORS[phase.difficulty] ?? ""}`}
                    >
                      {phase.difficulty}
                    </span>
                  )}
