...
function PlayerCard({
  ...
  return (
    <div
      className={`card p-4 flex items-center gap-3 transition-all ${isMe ? "ring-1 ring-white/20" : ""}`}
    >
      {/* Avatar + presence dot */}
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {player.isCompletedThisRound && !view.isWinner && (
            <span className="text-xs bg-green-400/20 text-green-300 px-1.5 py-0.5 rounded-full">
              Done ✓
            </span>
          )}
        </div>
      ...
      {game.status === "finished" && winner && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-6">
          <div className="card p-8 text-center max-w-sm w-full">
            ...
            <h2 className="font-display text-2xl font-bold text-white mb-1">{winner.name} wins!</h2>
